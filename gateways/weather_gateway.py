...
class WeatherGateway:
    ...
    @staticmethod
    def fetch_weather_summary(subject_location_summary, api_key=API_KEY):
    ...
    @staticmethod
    def _make_history_request(api_key, subject_location_summary):
        ...
        return ('http://api.wunderground.com/api/{api_key}/history_{date}'
                '/q/{latitude},{longitude}.json').format(
                    api_key=api_key,
                    date=subject_location_summary.date.strftime('%Y%m%d'),
